...
% Modified Feb 2016 by C. Kankelborg for use with GNU Octave (CCK comments below;
%     Matlab compatibility has not been verified).
...
% Note: We need two different versions, depending upon the version of Matlab.
        if exist("OCTAVE_VERSION") % CCK new conditional for GNU Octave.
            foo = @(x) chisquare_solve(x,P,V); % CCK
            X = fminbnd(foo,MINN,MAXX,optimset('TolX',TOLERANCE)); % CCK
        elseif (vers >= 6) % CCK changed from if to elseif.
